(function () {
    ...
    // Only run on pages that have Gravity Form #4
    ...
    var submitBtn = document.getElementById('gform_submit_button_4');
    if (submitBtn) {
    ...
    var ajaxFrame = document.getElementById('gform_ajax_frame_4');
    ...
    var fields = {
        vehiclePrice:   document.getElementById('input_4_23'),
        downPayment:    document.getElementById('input_4_24'),
        loanTerm:       document.getElementById('input_4_11'),
        tradeInValue:   document.getElementById('input_4_25'),
        amountOwed:     document.getElementById('input_4_26'),
        interestRate:   document.getElementById('input_4_27'),
        taxRate:        document.getElementById('input_4_28')
    };

    // Build the results container and insert it after the form wrapper
    var resultsContainer = document.createElement('div');
    resultsContainer.id = 'auto-loan-results';
    ...
    resultsContainer.innerHTML =
        ...
        '<div class="results-grid">' +
            '<div class="result-item">' +
                ...
                '<span class="result-value" id="result-loan-amount">--</span>' +
            '</div>' +
            '<div class="result-item">' +
                '<span class="result-label">Monthly Payment</span>' +
                '<span class="result-value" id="result-monthly-payment">--</span>' +
            '</div>' +
            '<div class="result-item">' +
                '<span class="result-label">Total Interest Paid</span>' +
                '<span class="result-value" id="result-total-interest">--</span>' +
            '</div>' +
            '<div class="result-item">' +
                '<span class="result-label">Total Cost of Loan</span>' +
                '<span class="result-value" id="result-total-cost">--</span>' +
            '</div>' +
        '</div>' +
    ...
    function validate() {
        if (parseNum(fields.vehiclePrice) <= 0) return 'Please enter a valid Vehicle Price.';
        ...
        if (parseNum(fields.taxRate) < 0)        return 'Tax Rate cannot be negative.';
        return null;
    }
    ...
    function calculate() {
        ...
        var vehiclePrice = parseNum(fields.vehiclePrice);
        var downPayment  = parseNum(fields.downPayment);
        var tradeIn      = parseNum(fields.tradeInValue);
        var amountOwed   = parseNum(fields.amountOwed);
        var rate         = parseNum(fields.interestRate);
        var taxRate      = parseNum(fields.taxRate);
        var termMonths   = parseTerm(fields.loanTerm);

        // Tax on the vehicle price
        var taxAmount = vehiclePrice * (taxRate / 100);

        // Net trade-in credit (trade-in value minus what you still owe)
        var netTradeIn = tradeIn - amountOwed;

        // Total loan amount
        var loanAmount = vehiclePrice + taxAmount - downPayment - netTradeIn;

        // Guard against negative loan amount
        if (loanAmount <= 0) {
            showError('Your down payment and trade-in exceed the total cost. No loan is needed!');
            return;
        }
